-- Mobile Optimization Database Schema (Fixed)
...
DROP TRIGGER IF EXISTS update_mobile_preferences_updated_at ON mobile_preferences;
CREATE TRIGGER update_mobile_preferences_updated_at BEFORE UPDATE ON mobile_preferences
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_offline_cache_updated_at ON offline_data_cache;
...
DROP POLICY IF EXISTS "Users can view their own sessions" ON mobile_sessions;
DROP POLICY IF EXISTS "Users can insert their own sessions" ON mobile_sessions;
DROP POLICY IF EXISTS "Users can update their own sessions" ON mobile_sessions;

DROP POLICY IF EXISTS "Users can view their own preferences" ON mobile_preferences;
DROP POLICY IF EXISTS "Users can insert their own preferences" ON mobile_preferences;
DROP POLICY IF EXISTS "Users can update their own preferences" ON mobile_preferences;

DROP POLICY IF EXISTS "Users can view their own notifications" ON push_notifications;
DROP POLICY IF EXISTS "System can insert notifications" ON push_notifications;
DROP POLICY IF EXISTS "Users can update notification status" ON push_notifications;

DROP POLICY IF EXISTS "Users can view their own analytics" ON mobile_analytics;
DROP POLICY IF EXISTS "Users can insert their own analytics" ON mobile_analytics;

DROP POLICY IF EXISTS "Users can manage their own cache" ON offline_data_cache;
DROP POLICY IF EXISTS "Users can manage their own pending actions" ON pending_mobile_actions;
